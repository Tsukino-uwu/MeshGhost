...
- `pseudo-hotreload.ps1` — the same idea for Pseudoregalia, but weaker by necessity: UE4SS
  ...
  `adapters/pseudoregalia/probes/probe_reloader`: write `<ModName> <nonce>` into its
...
launcher reads (staged from the root `config.json`'s client block plus
`packaging/config-overrides/<game>.json`, and the keys are `interp`, `curve`, `extrapolate`).
Nothing new was needed to make them per-game; what is missing is somebody sitting in front of each
game and
picking.
