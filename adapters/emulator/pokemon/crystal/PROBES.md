...
| `set_colour.lua` | **Writes object RAM and palette RAM**, held per frame, any build: puts an OBJ palette index on the LOCAL player (`MESHGHOST_COLOUR_PAL`, default green) and, with `MESHGHOST_COLOUR_RGB`, rewrites that slot's clothing colour (word 2 of 4) the way a build with a custom trainer colour does. Refuses to write unless the red slot's clothing colour reads as the game keeps it, which is its own check of the palette-RAM address. The other window's ghost is the verdict (`UNVERIFIED.md`, the trainer-colour entry, 2026-09-09). |
