...
| `substruct_order_probe.lua` | the **party in live RAM**: six copies of slot 0 re-keyed to each personality residue, one round per value in its `.cmd` file (2026-09-16) | No — `restore` in the command file or unloading puts the original bytes back; an in-game save while a round is written would keep the copies. |
...
| `find_objects.py` | Reads your ROM and a byte-identical build's symbols, not a running game. Lists a map group's character templates with their raw bytes, optionally only those with a nonzero field at an offset. How the first trainer for autoplay was found on route 0.17 (2026-09-16, `MEASURED.md`). |
...
| `fly_probe.lua` | What a FLY actually does, on BOTH sides at once. It reads the GAME and never the adapter -- its own task scan, its own sprite walk -- so "the adapter thinks X" and "the game is doing X" stay separable, which is what told a detection that never fired apart from one firing on the wrong thing. **Drives one instance from a savestate and observes from another** (`MESHGHOST_FLY_OBSERVE`), and **shoots a screenshot every fourth frame while any fly-bird is alive** -- keyed on the effect's own sprite callback rather than on a timer, so the frames land on the event. Twelve faults in one session had every struct field reading correct; the screenshots are what separated them. |
...
| `battle_state_probe.lua` | Read-only. A battle raw, on every change, with the pad: callback2, the battle's type flags, battler count and positions, both cursors, the outcome, each battler's controller routine, the battle script pointer and all of gBattleScripting, gAnimScriptActive and gPauseCounterBattle, the first two text printers, the four battler records with their names, and the battle message buffer decoded; a pad change alone logs a line. Read against captures of one wild battle, it is how autoplay's `battle` and its battle menus were measured (2026-09-16), the level-up box's waiting states (2026-09-17), and the animation that holds a battle's next message after STRING SHOT (2026-09-17, `MEASURED.md`). |
| `list_menu_probe.lua` | Read-only. List menus raw, on every change, with the pad: callback2, the bag's position struct, the plain menu struct, the shared list template, every active task's routine, and each list-menu task's data with its entries decoded. How autoplay reads the bag's item list, its scroll and pocket, and the item menu's grid (2026-09-17, `MEASURED.md`). |
| `trainer_approach_probe.lua` | Read-only. A trainer coming for the player, raw, on every change, with the pad: the approach globals, the script context status, the last-talked local id, callback2, the player's coordinates and the avatar block's first bytes. How autoplay's `walk` and `goto` learned to stop with `spotted`, and `battle` to wait while a trainer walks over (2026-09-17, `MEASURED.md`). |
| `move_data_probe.lua` | Read-only, logs once. For the moves in its list: the move table's 12-byte entry, the type name it points at and the move's effect text, for reading against the summary's BATTLE MOVES page (2026-09-16, `MEASURED.md`). |
| `bike_probe.lua` | Read-only. A ride frame by frame: the player object's coordinates, top bit and action byte, and the avatar block's first 16 bytes, on every change, with the pad. How the Mach Bike's speed field and its coast after release, and the Acro Bike's steady pace, were measured for autoplay's `walk` (2026-09-16, `MEASURED.md`). |
...
| `printer_state_probe.lua` | Read-only, no hooks. Text printers 0-7 and window slots 0-7 whole on every change, the bytes before each printer's pointer back to an FF, each window's first and last tile on its background beside its base block, how many byte values its pixel buffer's first 16 rows hold, and BG0's drawn rows. How a finished message after a restore was told from a stale printer: the window is put when its first and last cells hold its own tiles and something is drawn in it, and a field or battle message starts at its buffer's first byte (2026-09-17, `MEASURED.md`). |
| `task_probe.lua` | Read-only, no hooks. Every active task of the build's gTasks whole on every change (routine and 40 bytes), callback2 and the pad. Screens nothing reads keep their state in a task: how the wall clock's hours, minutes and steps were found (2026-09-17, `MEASURED.md`). |
| `naming_probe.lua` | Read-only, no hooks. The naming screen raw on every change, with the pad: callback2, the block its pointer leads to (the name so far at +0x1800, the page, state and cursor sprite id in the tail), the cursor's sprite, the template, and once the ROM's keyboard table decoded. How autoplay's `keyboard` and `type_text` were measured (2026-09-17, `MEASURED.md`). |
