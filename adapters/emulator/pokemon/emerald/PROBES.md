...
| `battle_state_probe.lua` | Read-only. A battle raw, on every change, with the pad: callback2, the battle's type flags, battler count and positions, both cursors, the outcome, each battler's controller routine, the four battler records with their names, and the battle message buffer decoded. Read against captures of one wild battle, it is how autoplay's `battle` and its battle menus were measured (2026-09-16, `MEASURED.md`). |
