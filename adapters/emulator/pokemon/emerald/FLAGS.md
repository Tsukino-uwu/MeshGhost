...
at the constants and in `VERIFIED.md`.
...
All but `MESHGHOST_NO_AUTOSTART` and the two `config.json` keys are **development-only**; those
three are supported player settings. None is set in a shipped release, and each falls back to a
...
| `"autostart": false` | `config.json`, the file the player already edits | absent — `true`, the adapter starts a core itself | The same thing as `MESHGHOST_NO_AUTOSTART` above, said in a file a player can see rather than a shell variable nobody can find afterwards (`meshghost_emerald.lua:1090`; the user's call 2026-09-03). Either one saying no is a no, and it is a **supported player setting** rather than a development switch. Every adapter honours the same key. |
| `"local_game_bridge"` | `config.json`, `"host:port"` | absent — walk from the base port | Where the bridge port walk STARTS, read out of the same file (`meshghost_emerald.lua:618`). `MESHGHOST_BRIDGE_PORT` is consulted first and wins; this is the player-visible way to say the same thing. |
...
| `MESHGHOST_EMERALD_REFL_TRACE` | global first, then environment (any value) | unset — off | **Probe.** The painted tier's walker-reflection decision line (`WALKER REFL ...`): what the ground test decided, per peer, on change and every two seconds. Until 2026-09-02 it rode on `MESHGHOST_COMPARE_TIERS` with ONE shared change key, so a crowd of 31 fired it ~1100 times a second and the user read the drawn tier as *"performance is chugging"* — the instrument was the cost. Now its own switch, keyed per peer. |
| `MESHGHOST_EMERALD_HOP_TRACE` | global first, then environment (any value) | unset — off | **Probe.** The ledge-hop state line (`HOP act=...`), four times a second per peer. Rode on `MESHGHOST_COMPARE_TIERS` until 2026-09-02 for the same reason as the row above; moved for the same reason. |
...
| `MESHGHOST_EMERALD_HW_OVERFLOW` | **global first, then environment; `"0"` disables** | unset — **ON since 2026-09-02** (user's call: *"we have watched OAM a lot, the only issues was that it can't be shown underwater while using dive, or in the foggy/mist areas"* — both are its recorded stand-downs, `VERIFIED.md` 2026-08-21) | **The hardware-sprite tier**, the middle rung of `spawn → OAM → drawn`. Peers the engine had no object slot for are given real GBA hardware sprite entries in `gMain.oamBuffer[64..119]` — the range above `gOamLimit` that the engine's per-frame path never writes and its VBlank transfer carries to the hardware anyway (`documentation.md`; the game parks its own wireless indicator at entry 125 for the same reason). The PPU then draws them, so they get **background priority and the live palette for free** — the occlusion and the fades the painted tier has to fake or simply lacks. Measured 2026-08-21 (`VERIFIED.md`): 56 hardware sprites cost nothing against a bare-emulator control, where painting the same 56 costs a third of the frame rate; spawned at its cap plus this at its then-ceiling, 67 characters, still 60.0 fps (a 2026-08-21, pre-pool-split measurement — since the five-way pool split the same day, the tier's peer-body ceiling is 26, see `BANDAGES.md`). **Vanilla only** — an Archipelago ROM relocates data, so there the tier declines and its peers fall through to the painted one, exactly like a tile-allocation failure does. It was off by default until 2026-09-02 pending a person's judgement; that judgement is now on record (see the default column). Global first so a loader script can flip it without restarting the emulator. |
...
| ~~`MESHGHOST_EMERALD_NO_BOBBER`~~ | — | **GONE from the source** | **Removed from the adapter, and this row records that rather than the flag.** It split the pair above so the surf blob and the underwater bobber could be told apart; it was the bobber, and `spawnUnderwaterBobber` is now `return nil` outright, so there is nothing left to switch off. The *method* — split a pair before choosing between them — is what survives, and it belongs in `probes.md`, not in a register of things you can set. Struck 2026-08-27, when the row still read as live and `grep` returned zero hits; the only trace left in the source is a dangling `-- see NO_BOBBER` comment beside `MESHGHOST_EMERALD_NO_BLOB`. |
