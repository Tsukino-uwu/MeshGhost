...
| Key | Type | Read from | Checked on arrival |
| --- | --- | --- | --- |
| `hurt_count` | running total | each drop of `CurrentHp` (`game_thread_tick`) | a number from 0 to 1,000,000,000 (a safety limit far past any real count), anything else reads as 0; acts only when it rises; a new ghost starts from the first value |
| `death_count` | running total | each time `CurrentHp` reaches 0 (`game_thread_tick`) | a number from 0 to 1,000,000,000 (a safety limit far past any real count), anything else reads as 0; acts only when it rises; a new ghost starts from the first value |
