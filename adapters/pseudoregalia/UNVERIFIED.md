...
- READY — **The driven ghost's PRIVATE game instance, the side-by-side and the Power mapping (build `eb2edd4701c8`, both installs, 2026-09-09 00:1x).** (1) The hurt-variant sit (your words 00:2x: *"it also sits as if its hurt/low health"*; `CurrentHp` lives on the shared game instance the ghost is cut from on purpose): the driven ghost now gets an object of the instance's own class, every plain-valued field copied from yours, in the nulled ref's place (`private_gi=1` in `ghost_drive.txt`; `0` is the A/B). Correct: the sit on the chair is the HEALTHY sit, the one you get at full health, and your own health bar does not move when the ghost sits. The log's `DRIVE ... private game instance ... CurrentHp player=N ghost=N` line says what it copied. (2) `replay/active/` holds a second copy of the sit clip named `mirror` (orange tag) and `mirror_offset=0,250` draws it 250 units to the side of the driven ghost, mirrored the shipped way -- the 1:1 side-by-side: the driven ghost (your tag) should walk, sit, stand and face on the SAME frames as the orange one, 250 units apart; a lag, a missed action or a visible correction snap is the finding. (3) A Power press in a recording fires IA_Power node `_1` (a GUESS by the Attack analogy, logged as `DRIVE ... Power press -> node _1`); if the ghost does not do what you did, `power_press=0` or `2` in the file is the next try, live. Also still yours from the night: the stand-up on the sit clip (moving the stick after the sit stands the ghost up, as it did you), and whether anything about the driven ghost reads as a bandage.
- OPEN — **The hurt-variant sit is NOT the health value** (2026-09-09 00:2x-00:57, your words at 00:27: *"Mirror looks hurt"* with `CurrentHp` 80 on the ghost's own private instance, logged both sides): `probes/probe_pawndiff/` found the driven pawn differs from yours in the save's UPGRADE fields (`healUpgrades` 2/0, `damageUpgrades`, `powerBuildUpgrades`, `powerMeterUpgrades`, `bonusAirKicks` 4/0, `healAmountPerDing` 20/10, `canMoveHeal?`, `canDoAirRecovery?`), and `copy_config.lua` wrote those eight onto every driven pawn from 00:30 to the close at 00:57, read back correct each loop. **Your word on the sits after 00:31 is owed**: healthy means the sit reads the upgrade set and the copy moves into the DLL's prepare; still hurt rules that group out and the search widens to what the sit calls (the HUD ref, the instance's object fields, or the chair's own actor).
