...
- READY — **The input track carries the CAMERA now (2026-09-08 evening build, ADR 0057's Stage 0): six axes on every edge, `cam_yaw` and `cam_pitch` last, from the controller's `ControlRotation`; `source` reads `imc_keys+bound_axes+camrot`.** What the first run has to show, none of it on screen: `UE4SS.log` has `INPUTTRACK: ControlRotation resolved (size 24)` (a `camera refused` WARNING instead means the property is not where the engine source says, and the track then says so in its `source`); the new `replay/inputs/in-*.ndjson` header lists six `axes`; and while you turn the camera a full circle standing still, the fifth `ax` value walks through 360 degrees of yaw and the sixth moves only when you look up or down. Which way is positive is recorded from the file, not assumed. Nothing reads these yet; they are for the driven ghost.
