...
- OPEN — **CHASER GHOSTS ARE INVISIBLE AFTER A DEATH AND RESPAWN (the user, on screen, 2026-09-18; logged at their request, NOT investigated).** Seen on the build of that evening (`main.dll` `89ecb51e075d62e8`), during the damage-path session: the user died to an enemy, respawned, and the chaser pack did not come back visibly. Nothing here is a diagnosis — no probe was pointed at it and the session moved on. **What is already known that a hunt should start from, all measured the same evening:** a death makes an **entirely new pawn** (the watcher saw `As MV Game Instance Ref` read `nil` for one 25 ms sample across the swap, and the fresh pawn's own `BP_HpHitable` read 20 before settling to 80), and this adapter already releases every ghost at `InitGameState PRE` — which is exactly the "same-level retry/save reload" path a death respawn plausibly takes — then holds `suppress_ghost_spawn_until_tick` for a window afterwards. So "released on the respawn and never respawned" and "respawned into the suppression window and dropped" are both live candidates, as is a chaser-pack clock that never restarts. **Honest note on this build:** its one chaser-specific change is the hurt-reaction skip below, which only declines to call `BPI_PerformDamageResponse` and touches no visibility, spawn or lifetime path — so it is not a plausible cause, but it IS the change that shipped alongside, and a bisect should rule it out rather than reason it away. Worth checking whether peer/replay ghosts survive the same respawn, since that splits "chasers specifically" from "all ghosts". **Seen again 2026-09-23, and the log names a lead (not yet a cause):** the user died, the death reloaded the level (`LoadMap PRE`), the adapter respawned all five chasers into the new world at once, and five seconds later (the chaser delay) `chaser:1` reached the death in the recording and the adapter ran the pawn's own `dieFade(true)` on it (`MIRROR_DEATH_FADE`). The matching `dieFade(false)` rides only the `rsp` respawn effect, and `rsp` never fired in that session, so nothing un-fades them. The user: *"after i respawned, i only saw the shadow on the ground on chaser ghosts and nothing else"*: the model faded and the shadow stayed. Before a fix: ask what the player's own model does on screen when it comes back after a death, and settle whether a peer ghost has the same gap. **The chaser half is FIXED and user-confirmed (2026-09-23, `VERIFIED.md`, "a death starts the chaser pack over"): a reset pack never replays the death.** Still open here: whether ANOTHER PLAYER's ghost comes back after its own death fade (needs two clients).
- OPEN — **"Fatal world leaks detected" on "restart last save", right after the invisible-chaser state above (2026-09-23 01:09, the first since 2026-08-13).** The adapter's `LoadMap PRE` release of all five chasers logged cleanly; the crash followed. Shipping build, so there is no engine log naming the reference chain. Two lab probes had called `BPI_TryDamage` on the player that session and were unloaded before the reload. Unmeasured whether it reproduces, and whether a chaser that ran `dieFade(true)` is what the old world was kept alive by. Next: die with chasers up, then restart last save, first on this build, then with the chaser death fade subtracted.
- OPEN — **Wall-kick effects on a REPLAY ghost look off in position and timing, maybe on other ghosts too (the user, 2026-09-23; logged at their request, not investigated).** The 2026-09-01 wall-kick mirror entry further down stated one limit in advance: `wk` (`NS_WallKickHit`) spawns centred on the ghost, not at the wall face, and the fix it names is a world-frame offset carried on the wire, never guessed from yaw. Timing was not a stated limit. First: which ghost kinds show it (replay, chaser, peer), and whether it is the burst (`wk`), the flourish (`ks`) or both. **The user's guess, 2026-09-23: the same shape as the replay jump dust** (the OPEN "world-spawned VFX outlived the ghost" entry: dust in wrong positions after a replay restarts or loops). Worth checking first because `wk` shares that path: a world-spawned one-shot, placed by the same observed-height learning (`observed_world_offset_z`) the dust uses.
- OPEN — **THREE GHOST-ATTACK LEAKS the user still sees with `GHOST_PREHIT_PLAYER` on, and the chaser contact plan behind them (2026-09-15, nothing built on this side yet).** The user's report: **Sunsetter** hurts the player and other ghosts, **Strikebreak** hurts the player and other ghosts, and ghost attacks **hit switches and levers** — three paths the pre-marked `hitActorsArray` never closes, which breaks the contract's "a cosmetic ghost never damages or interacts" before any contact mode can mean anything. The plan is `agent_docs/chaser-planning.md`: leaks first, then how the game hurts and kills the player, then a freeze during NPC dialogue and note reading, then the adapter's contact test. **The Go side shipped the same day** (ADR 0068: `chaser.contact` is `"off"`, `"hurt"` or `"kill"`, and `session_policy.chaser_contact` carries the word); this adapter parses neither `session_policy` nor `render_remote.cosmetic` today, so its first step is a handler, not a fix. Three facts about the repo, checked against the code on 2026-09-15: `player_frozen` IS sent by this mod since 2026-09-05 (pause menu and item popup, `PauserPlayerState`), so the 2026-09-04 "blocker" entry further down is answered and only the I6 watch item (item 5 in the list above) stays open; `render_remote.cosmetic` IS on every chaser frame; and no grace window exists anywhere, so the post-seam window is this adapter's to measure. **A tester's lead, unmeasured:** everything health-related goes through `BP_HpHitable`, and every other health value (the GameInstance `CurrentHp` that `documentation.md` names as THE field) is a copy kept for convenience — if so, hurt and kill go through a `BP_HpHitable` function, never a field write, and `documentation.md`'s two homes for `CurrentHp` (lines ~325 and ~724) are one object mirrored. The first instrument is written and parked: `probes/probe_hitlist/` (whose already-hit list records a victim, and where the health moves), never loaded yet. Nothing here is a claim; every line above is what to measure.
- OPEN — **A peer's costume that the watcher OWNS but has never WORN this session is unresolvable forever, and the retry is unbounded (measured 2026-09-13, not fixed).** In the 13:45 session ghost `p2` cycled six costumes; `Fierce_Deity`, `Hornet`, `IRA` and `kate` applied, then `Kindred` and `KOS-MOS_Ver_1` never resolved — **1,798 `not found via StaticFindObject` warnings over 20 minutes**, continuing long after the ghost went quiet. Both `.pak`s are installed in that install; I diffed the two `Content/Paks/Outfits` folders and they are identical. The cause is stated in `resolve_peer_named_asset`'s own comment: the catalog enumerates **loaded** objects, and a costume pak's mesh does not load until that costume is worn locally. Two costs: the ghost silently keeps the previous costume (looks like "the ghost did not change outfit"), and the failed target never advances `last_synced_outfit_mesh`, so it retries ~1.5×/s forever, each with an `Output::send`, plus a full `FindAllOf("SkeletalMesh")` walk every 10 s. **Deliberately left alone for now** — it is a separate bug from the stranded-materials one above and mixing them into one build would make a decline unattributable. Worth deciding: give up after N attempts and say so once, and/or find a way to make the asset load.
- DONE — **THE OUTFIT ECHO IS THE COSTUME MOD'S SAVE FILE, SHARED BY TWO GAMES ON ONE PC -- not MeshGhost, and unreachable between two machines (measured 2026-09-13).** The user: *"the player swaps, and a bit after the other player swaps into that same outfit"*. A logging pre-hook on the native `SkeletalMeshComponent:SetSkeletalMeshAsset` (`probes/probe_outfitswap/Scripts/echo_hook.lua`) printed every call into `UE4SS.log` beside the adapter's own lines. In the swapping game, each menu pick set the player AND a `ModActor_C...CharacterMesh` (the costume mod's actor). In the watching game: 19:34:19.142 the adapter applied KOS-MOS to ghost `p4`; 19:34:20.006 the replay ghost spawned; **19:34:20.052 the PLAYER's VisualMesh was set to KOS-MOS with no adapter line beside it**. The value came from `%LOCALAPPDATA%/pseudoregalia/Saved/SaveGames/UI_Overhaul File 7.sav`, which holds `currentOutfit` (read: `Saeksuru` after the user's last pick) and `OutfitMeshesList`. That folder is per Windows user, not per install, so two local games share it: one saves the pick, the other re-reads it when any player-class pawn spawns and applies it to its own player. The earlier `echo_source.lua` negative fits (a ghost set locally never reaches the file), and so do the reinstall and the two-install history. **Nothing to fix in the adapter.** For two-game testing on one PC: `agent_docs/running-the-rig.md`.
- READY — **RESTORED 2026-09-13: the stranded-materials fix was reverted over a regression that turned out to be the costume mod's shared save file (entry above), so it is back in the source, matching v1.2.9. Its own purpose is still UNWATCHED.** Original entry follows. The user, after release: *"whenever a player swap outfit it applies to other clients/players now, not just the ghosts"*. The `Plugin.cpp` half of `d133352b` is reverse-applied and rebuilt (probes and records kept); v1.2.9's DLL had no other behaviour change, so the revert is the bisect. The mechanism is **unmeasured** — the leading theory: with the overrides cleared, the ghost renders the costume asset's SHARED MaterialInstanceConstants, so anything that writes parameters through `GetMaterial(i)` without making a dynamic instance first writes the asset every pawn in that costume draws. Next attempt must be judged with two real players BEFORE a release, and should rebuild each slot as a dynamic instance owned by the ghost (`CreateDynamicMaterialInstance` exists on this build, `meshfns.lua`) rather than leaving it empty. The original finding below stands. — Was: **THE OUTFIT-SWAP-DURING-HURT GLITCH IS STRANDED MATERIALS, and the engine said so (built, deployed to both installs, hash `8a55b907d2b953dd`, UNWATCHED).** Your repro, 2026-09-13: take damage, swap costume during the hurt/blink, and the watcher's ghost keeps a mangled model that survives every later swap and clears only on a save reset. **Measured on the live glitched ghost** (`probes/probe_outfitswap/Scripts/materials.lua`): it wore `Kindred` while all four of its material slots rendered MaterialInstanceDynamics parented to `KrystalBody`/`KrystalFace` — the costume it had worn seconds earlier. The local player, dumped in the same pass as the **control**, wore its own costume with zero foreign slots, which is what makes this a defect rather than an engine detail: a dynamic instance per slot is ordinary, one parented to a *different* costume is not. The chain is all in that session's `UE4SS.log`: outfit applied (Krystal) → you were hurt, so `MIRROR_HURT_REACTION` runs the game's own `BPI_PerformDamageResponse` on the ghost → the flash builds a dynamic instance per slot over the costume worn **at that moment** and parks them in the component's `OverrideMaterials` → outfit applied (Kindred) swaps the MESH out from under them. `OverrideMaterials` belongs to the component, not the mesh asset, so nothing in the swap touched it — and the log still said `outfit mesh applied` with a matching readback, because the readback reads the property that was written, never the render state. Your own character never shows it because the costume mod's own swap path rebuilds them. **Fix:** clear `OverrideMaterials` inside `call_set_skeletal_mesh_asset`, before the setter — which covers the WEAPON path too, since it shares the recipe; it measured clean only because no weapon was swapped during a flash. `Reset()` not `Empty()`: `Reset(0)` takes its no-reallocate branch and only zeroes `ArrayNum`, never calling the allocator, which is the line this file already draws around growing a UE TArray from this DLL. **What is yours to say:** run that exact repro — damage, then a costume swap during the flash — and the ghost wears each new costume with its own textures however the swap is timed. A `dropped N stranded override material(s) before the mesh swap` line in `UE4SS.log` says it fired. **Unmeasured and reasoned only:** the afterimage trail builds from these same materials, so a glitched ghost's trail colours should come right too — worth one look, not a claim. Two theories this falsified and why they looked right are in the probe comments (the anim binding diverges on the *player* too, identically, and looks fine). See also the 2026-09-05 OPEN entry below (*a remote ghost came back GLITCHED after its peer's reset-to-save*): same visual family and the same hit-then-mesh-rebuild shape, but a different trigger and never reproduced — **this is a candidate explanation for it, not a closure of it.**
- READY — **THE ONE-SHOT BURST RING SURVIVED A LEVEL CHANGE, and it is the FIFTH instance of the family the code's own comment predicts** (`release_all_ghosts`, built and deployed to both installs, hash `BEBC2023`, UNWATCHED). That function clears every raw pointer a `RemoteGhost` holds -- `vfx_components`, the weapon prop, the projectile, all three nametag handles -- and never touched `recent_one_shot_components`. `release_ghost` clears it and says why in as many words; this one runs at LoadMap PRE, at **InitGameState PRE (a same-level "retry last save")** and at the pause-menu reset, and left the ring full. It is not a use-after-free, which is why it survived four passes over this exact family: the ring is EXPECTED to hold freed pointers (a burst destroys itself when its particles finish and nothing tells us), so `destroy_world_spawned_components` checks every candidate against `FindAllOf` first. What that cannot see is an address the level RECYCLED -- and then a later despawn destroys a live component belonging to somebody else, while the detection pass's "ours, do not measure" set excludes a component that is not ours at all, so the LOCAL player's own effect quietly stops being mirrored. **What is yours to say:** after a `retry last save` or a zone change with ghosts up, effects still start and stop on the ghosts as before, and your OWN effects still mirror to the other client. Found by the growth cell of the third adversarial review (P2e-2, 2026-09-12).
- READY — **A clip's input track could no longer make the game do something undefined** (same build and deploy, UNWATCHED). `remote_input` parses `f` and `t` out of JSON as doubles and narrowed both with a bare `static_cast`; a value outside the destination integer's range makes that undefined behaviour, in your game process, from a clip a friend sent you. `m` on the very next line was already checked for finiteness AND range, which is what makes this a missing guard rather than an unconsidered case. Both are now bounded and an out-of-range edge is dropped rather than clamped -- an input applied at an arbitrary moment is not a "bounded wrong value" the way a colour channel is. The Go side was fixed the same day and is the real close (`bridge.MaxInputFrame`), so this is belt as well as braces. **What is yours to say:** an ORDINARY driven-ghost run still behaves exactly as it did -- this must be invisible, and an edge going missing would look like the ghost skipping an action. Found by the growth cell of the third adversarial review (P2e-1).
- READY — **A TESTER'S CRASH WAS OURS, and the cause is named: the mirrored-VFX map kept a component of a ghost that had already been destroyed (crash 2026-09-10 18:28 UTC, chaser pack of 7 in `Zone_Tower`; fixed and deployed to both installs, UNWATCHED).** Their dump symbolizes to `game_thread_tick -> tick_remote_mirrored_vfx`, `Plugin.cpp:14194` -- the `Deactivate` in the effect-STOP branch -- 5.7 s after the redraw loop logged `remote chaser:1 ghost is no longer valid (level transition) -- releasing stale reference, will respawn fresh.` and respawned with no release in between, so `vfx_components` still named the dead actor's Niagara component. Both redraw drop sites now clear it (and `weapon_fly_component`) the way `release_ghost` and `release_all_ghosts` always have; a new preflight section checks every `.ghost = nullptr;` site, negative-tested. **A SECOND dump from the same tester (19:20 UTC, a different session on the same build) is the same fault one call further in** -- `Plugin.cpp:14198`, the `DestroyComponent` lookup after the `Deactivate` one, again 14 ms after that same "no longer valid" line for `chaser:1`. Two dumps, one cause, one fix. **What is yours to say:** a long session with a chaser pack across several zone changes stays up, and the effects on a chaser still start and stop as before. The diagnosis is from the dump and the logs, not from screen -- the FIX is what waits on your eyes. `pitfalls/by-lesson.md` 2026-09-10, `phases/phase7.md`.
...
- READY — **The driven ghost's chair sit is the HEALTHY one now (build `0f7a541e7d27`, both installs, 2026-09-09 01:1x; the Lua prototype user-confirmed 01:05: *"its not hurt anymore"*).** The cause: the spawn decouple nulls the pawn's reference to its OWN health component (`BP_HpHitable`, a child of the pawn: CurrentHp/maxHP), and the sit chooses hurt/healthy through that reference -- health on the game instance (80, logged both sides) and the save's upgrade fields both left it hurt. The DLL now stashes the ghost's own component at decouple (own outer only, logged) and hands a DRIVEN ghost the reference back at prepare, reading CurrentHp back through it (`DRIVE: ... own BP_HpHitable restored -> ... CurrentHp=80`). Also in this build, unwatched: **a level reload (your "reset last save") re-prepares the respawned driven pawn** -- at 01:04 the reload kept the RemoteGhost, so the new pawn ran unprepared for 24 s (no collision, no facing, no private instance, 16 corrections per 10 s: your *"it snaps instead of moving smooth and it also looks hurt when sitting"*); a spawn now resets the prepare. Correct after a reset: the ghost walks the route as before the reset and sits healthy. `private_gi=1` stays (harmless, not the cause); `mirror_offset` and the Power guess (`_1`, fired twice per loop, unjudged) are unchanged. The rig is DISARMED in the install (`ghost_drive.txt.disarmed`); rename it back to arm.
- READY — **THE DRIVEN GHOST STANDS UP (user-confirmed on the Lua probe 2026-09-09 12:0x: *"yee it stands up after being on the chair now"*; the C++ rig's own copy of it, `stand_fn` default `change Move State:0` plus `stand_stop_blend` 0.2, is built and UNWATCHED -- your word on the next launch is what moves it).** The game's stand-up is two things, and it took both: the pawn's own `change Move State(0)` on the stick's rising edge while seated (moveState 8 -> 0, MovementMode 5 -> 1 the same tick) and `customStopMontage(0.2)` right after -- the sit is a montage, `dreamLady_Sit_Montage`, that the state change alone leaves playing (your *"still stuck in the sitting pose after leaving the chair"*, 12:0x); on your own two stands it was still blending at +150 ms and gone at +200 ms, and the ghost reads the same after both calls. Six candidates from the interact/sit/heal census did nothing (`EndInteract`, `BPI_EndInteract` on the pawn and on the chair, `healDing`, `tryFinishHeal`, `exitTransition` both ways, 11:45-11:52); the unfiltered function-name dump named the setter. The chair GLITCH is the state leaving 8 with the montage still playing, measured on you at 12:00:06 -- that path has no stick edge, so the ghost's stand-up never fires there. `probes/probe_pawndiff/Scripts/standup_hunt.lua`, `pawn_census.lua`; `phases/phase7.md` 2026-09-09 (midday).
- READY — **THE DRIVEN GHOST ON A ROUTE WITH CLINGS, SUNSETTER AND A PIT FALL (2026-09-09 12:1x-12:5x, your recordings `rec-20260909-121043` and `-123724`; five builds, the last three UNWATCHED).** What you said, in order: *"it was snapping a lot"* (8 corrections a loop, all in the clings) -> after the input gate *"looks better while using cling on a wall now, but snaps a lot whenever you are hitting a wall while gliding"* and *"the shadow is following the ghost model"* -> shadow fixed (*"the shadow works now"*), *"still snapping a bit on the 2nd cling"* -> *"still snapped while using cling on the wall, and also while using solar wind i think. but sunsetter looked a bit better ?"*. Built since your last word, in this order, each measured before it was written: **(1)** a correction sets the recording's velocity instead of stopping the pawn (the stop zeroed every Sunsetter plunge within 0.1 s and restarted every wall slide; `snap_stop`); **(2)** the save's numeric upgrades and `can*` bools copied at prepare (`bonusAirKicks` 4 against the clone's 0); **(3) THE BOUND STICK** (`bound_stick`, DLL `868a2707d5f1`, see `FLAGS.md`): the pawn's Blueprint reads the move stick through Enhanced Input's bound value, zero on a clone, so every stick-dependent mechanic ran stickless -- the wall slide fell at full speed where you eased down, the wall run took no direction, slide jump and Solar Wind had no steer; a post-hook on the native getter now answers the driven ghost's `IA_Move` reads with the recorded stick and the rig's own engine-level push is off. Yours to say on the next launch: the clings, Solar Wind and the slide jump, against the rig's `DRIVE ... correction #` lines (13-17 a loop on the `123724` clip before the hook). Still known and unfixed: **the pit fall** -- your character's `respawnTransform` holds the safe spot and the clone's stays at zero (measured 12:44, both pawns every 2 s), so the clone's reset lands at the world origin for a frame before the correction moves it to your safe spot. The writer is a PLACED VOLUME: `BP_SafeZones_C`, 78 of them in the loaded world (`safezone_census.lua`, 12:5x, a full class census filtered on reading), and the driven pawn's capsule has overlap events OFF and is typed WorldDynamic, so no safe zone ever sees it; the pit itself (`BP_HazardZone_C`, 31) does reach the clone, since it died. Not changed: turning overlaps on for the clone would also hand it every pickup and door, which is game state (`CLAUDE.md`). Yours to decide whether the origin frame matters on screen. `probes/probe_pawndiff/Scripts/snap_watch.lua`, `input_gate.lua`, `wallrun_entry.lua`, `safezone_census.lua`; `phases/phase7.md` 2026-09-09 (afternoon).
- OPEN — **"Two ghosts" with one clip (your 11:5x screenshot, `Tsukino-ghost`) was the LOOPBACK relay echoing you back -- your call, right. Not the stale seated pawn: the census reads that one at `0,0,0`, flagged being-destroyed, no controller, gone within two loops -- invisible, one pawn's memory until the next collection.** Left here until you say a driven-ghost session with the plain relay shows one ghost. Restarting the relay under a live game crashed it once (11:51, the effect-cleanup diagnostic on a freed candidate, fixed in `3d5f0949`); a relay restart with a peer attached is otherwise a leave, and worth watching once with the fix.
- OPEN — **Stale driven pawns outlive their loop** (measured 00:29-00:31, `pawndiff-002922.log`): five `BP_PlayerGoatMain_C` with one player -- the driven ghost, the offset mirror, and TWO flagged `bActorIsBeingDestroyed` that stayed through every 4 s snapshot for over 20 s, one of them SEATED (`moveState` 8, no controller, a private instance of its own). A destroyed-flagged pawn kept alive is the chair holding its occupant (`Interaction Target` is never cleared -- `sit_watch.lua`) or the known no-op destroy on this build (host `CLAUDE.md`, "Destroying the ghost pawn"). Unmeasured which; the cost is one pawn per loop while it lasts. Not shipped behaviour -- only a driven ghost sits.
...
- DONE (user-confirmed 2026-09-11, `VERIFIED.md`) — **the recording indicator is a SCREEN-SPACE widget now (build `0e1c5dc9f3d9`): built once per recording, handles valid on the tick they were made, removed on stop -- in your 15:16 screenshot at 0:00. Yours to say: it looks like the ShareX-checked prototype and keeps its time across a zone change.**
...
**A CANDIDATE EXPLANATION ARRIVED 2026-09-13, and it is not a closure.** The stranded-materials bug
(the READY entry at the top of this file) is the same visual family — a hit, then the mesh changing
underneath the dynamic materials the hurt flash had just built, leaving a body that renders as
fragments — and it matches three details here: the red flash immediately before, the persistence, and
a watcher-side world reset being what cleared it. What does NOT match is the trigger: there the mesh
changed by a costume swap, here by a despawn/respawn, and a respawned ghost should get a fresh
component with an empty override array. So this stays OPEN. **If it recurs, run
`probes/probe_outfitswap/Scripts/materials.lua` against the broken ghost before doing anything else**
— it prints each slot's material and what it was built from, with the local player as the control,
and one pass answers whether this is that bug or a second one.
...
**Asked for 2026-09-23 (the user): an audit, not only this fix.** Every call site that spawns a
world-space effect (`spawn_niagara_at_location` and any other free-standing spawn) gets its
lifetime and position traced against despawn, replay loop/restart/rewind, a level reload and a
death, so nothing else of this shape is left. The wall-kick report above is one candidate.
...
because the fault was an accumulating DRIFT and the action that produced it was not exercised** —
not because the wording was soft. **Trigger that releases it:** the next Pseudoregalia session in
which somebody deliberately falls past their own lingering dust. If that happens and the dust is
still where it belongs, this drains without further ceremony.

(Re-examined 2026-09-10 against the narrowed settle rule: a positive statement confirms, and a hold
may not rest on coverage alone. This one survives on the intermittent-fault clause, which is the
half of that rule that still stands — but it was missing its trigger, so nothing would ever have
released it.)
