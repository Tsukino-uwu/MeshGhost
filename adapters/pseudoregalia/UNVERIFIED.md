...
- OPEN — **The driven ghost never STANDS UP (your words 01:05: *"it keeps sitting after getting of the chair"*; the DRIVE TRACE: moveState 8 from the clip's sit until the loop seam, while the clip says 0).** The game's own stand-up is the rising edge of movement input while seated (`sit_watch.lua`), run by a handler that reads the BOUND stick -- zero on a clone. `EndInteract` on the pawn (no parameters) was called on every rising edge by the Lua prototype (`probes/probe_pawndiff/Scripts/standup_edge.lua`, 01:08) and moveState stayed 8: ruled out. Build `0f7a541e7d27` adds `stand_fn=<function>` to `ghost_drive.txt` (called on the stick's rising edge while seated, no-arg functions only, moveState logged same tick) and logs the candidates' signatures once per session (`DRIVE: stand-up candidate ...`): `BPI_EndInteract`, `exitTransition`, `tryFinishHeal`, `healPlayer` are next, one per launch, read from that log first. A level trigger is exactly wrong for the table glitch (seated walk = no edge); keep it edge-triggered.
