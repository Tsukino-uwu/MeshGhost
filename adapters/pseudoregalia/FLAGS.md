...
`Plugin.cpp` carries 101 `constexpr bool` switches (re-counted 2026-09-08 with `grep -c "constexpr bool "`, after the three input-track flags and `REC_INDICATOR_SCREEN_SPACE`;
...
## Behaviour — the 36 that are `true` (re-counted 2026-09-08, after `INPUT_TRACK_CAPTURE`, `INPUT_TRACK_AXES` and `REC_INDICATOR_SCREEN_SPACE` were added)
...
days, which is why the figure carries a date and the COMMAND, never a total to trust): **101**
`constexpr bool` declarations in `Plugin.cpp` (2026-09-08: 97 plus the three input-track flags plus `REC_INDICATOR_SCREEN_SPACE`), being 100 written plus
