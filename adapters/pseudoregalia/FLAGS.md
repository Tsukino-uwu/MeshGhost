...
| `INPUT_TRACK_AXES` | **Added 2026-09-08 with the flag above.** Four axes on every edge (`move_x, move_y, look_x, look_y`), quantized to 1/64 and throttled to `INPUT_TRACK_AXIS_MIN_MS` unless a button edge carries them: move from `IA_Move`'s bound value (the one action this pawn value-binds), look from `GetInputVectorKeyState` summed over the keys the applied table binds to `IA_Look` (stick position plus mouse delta -- the third run read the bound value as 0 for a whole run of looking). `false` sends buttons only. |
