...
`Plugin.cpp` carries 102 `constexpr bool` switches (re-counted 2026-09-08 with `grep -c "constexpr bool "`, after the three input-track flags, `REC_INDICATOR_SCREEN_SPACE` and `INPUT_HISTORY_DISPLAY`;
...
## Behaviour — the 37 that are `true` (re-counted 2026-09-08, after `INPUT_TRACK_CAPTURE`, `INPUT_TRACK_AXES`, `REC_INDICATOR_SCREEN_SPACE` and `INPUT_HISTORY_DISPLAY` were added)
...
| `INPUT_TRACK_CAPTURE` | **Added 2026-09-08, the adapter half of ADR 0056 (the input track), UNWATCHED.** Once per engine frame, in the local-state section of the tick, reads the local player's input and sends the CHANGES to the core as `input_sample` edges; the core writes `replay/inputs/`. Two reads, and the first live run (13:15) decided the split: **buttons** through `APlayerController::IsInputKeyDown(FKey)` once per key the engine's APPLIED merged table (`PlayerInput.EnhancedActionMappings`) binds to each of 11 actions -- not every loaded context: `IMC_Reference` is the factory defaults kept in memory beside the live `IMC_Default`, and merging both put a jump on every gamepad attack in the 13:36 run (every offset from reflection, the FName bytes copied from the game's mapping into a zeroed FKey, the table rebuilt on a cadence so a rebind lands live); **sticks** through `UEnhancedInputLibrary::GetBoundActionValue` on the pawn. The first build read the buttons through the bound value too and got nothing — a bound action value exists only for actions the Blueprint binds by VALUE, which on this pawn is the sticks (`setInputVariables`, `poleTick`); the buttons are event-bound. **Compiled in and `true`; the runtime gate is config.json's `replay.inputs`, which ships `false`** — the plan had written this flag as `false`, and that was changed so the config key is not a lie (a compile-time off would make the player's setting do nothing). The label order is the `BUTTONS` array in `input_track_sample` and the `labels` string in `input_track_drain_and_send`, which must agree: `jump, attack, crouch, wallride, throw, guard, interact, lockon, power, quickmap, perspective`. Self-checking: both reflected calls' parameter and return layouts are verified by name (and the FInputActionValue by size) before the first call (a mismatch refuses that half with a WARNING; the other half keeps recording), and every jump edge is compared to the pawn's own `jumpButtonHeld?` — the `INPUTTRACK:` log line's `agree`/`disagree` counters, printed every tenth bridge line. Source decided by the 2026-09-08 census, `probes/probe_inputcensus/`. |
| `REC_INDICATOR_SCREEN_SPACE` | **Added 2026-09-08, UNWATCHED in C++ (the mechanism user-confirmed on the Lua prototype the same day).** The recording indicator is two UMG UserWidgets constructed at runtime (a Border each, the clock's holding a TextBlock) and added to the viewport, instead of two TextRenderComponents placed in the camera's frame: composited over the scene, so nothing hides it, and laid out in screen space, so a field-of-view change cannot move it. `false` is the old world-space path, whole. Numbers: `hud_x hud_y hud_size hud_gap hud_text hud_pad hud_z` in `rec_indicator.txt`, defaults the user judged (26/22/44/10/30/4/0 (the z-order was 1000 until a tester found it over the pause menu and in front of its mouse input, 2026-09-08) at 1920x1080). Pinned in the root set while shown; rebuilt after a transition. |
| `INPUT_HISTORY_DISPLAY` | **Added 2026-09-08, UNWATCHED in C++ (the look user-judged on the Lua prototype `probes/probe_inputdisplay/`).** The player's inputs as an on-screen history: a UMG panel (Border + TextBlock, the indicator's helpers) with one row per held-state, the frame count, direction arrows from the move stick's bound value and one letter per held action (`JACWTGILPMV`), newest on top. Fed by the input track's own per-frame read, which now runs when either consumer wants it (the queue is fed only when the track does). Runtime gate: config.json's `input_display` section -- `player`, `background`, `rows`, `size`, `player_side` -- shipped off (`always`, which gated the player's panel on a running recording, was removed 2026-09-09: a tester had to set both and found it confusing; the panel shows whenever `player` is on). **The ghost half is BUILT (2026-09-08 evening, ADR 0057) and SEEN by the user across seven passes:** `ghost` and `ghost_side` in the same section; the mod's hello carries `input_tracks`, the core streams a replay's track as `remote_input`, the mod buffers it per ghost and applies each edge on the first `render_remote` whose `state.timestamp` reaches the edge's `at` (the first time this adapter reads that field), then draws the state on a second panel of the same code (`InputPanel`, one per source) that follows the first replay ghost with a track, on `ghost_side` or the player's side when the player's panel is off, and survives a loop seam for ~3 s. A `reset` line clears the rows. One empty panel on the pre-trace build (22:07 run, one of eight restarts that evening) is unexplained; `INPUTDISPLAY: ... first edge applied` / `waiting` in `UE4SS.log` is the trace that would name it. `false` compiles the display out and stops the hello asking, so the core scans for no track. |
| `GHOST_DRIVE` (the dev rig; `ghost_drive.txt`) | **Added 2026-09-08/09, ADR 0057's stages D1 and C, SEEN by the user as a dev rig, nothing shipped.** With `ghost_drive.txt` beside the DLL, the first replay ghost is DRIVEN instead of mirrored: its whole mirror section, the pose teleport, the animation-state writes, the montage start/stop/divergence corrector and the landing/jump pulse are skipped for it; the tail's cosmetics (sword, outfit, nametag, effects) still run. Prepared once per pawn: collision ON with the capsule ignoring Pawn and Camera, typed WorldDynamic, no overlap events; `bOrientRotationToMovement` back on; every `obtained*`/`has*` bool copied from the player's pawn (a clone has class defaults -- `obtainedAttack?` false refused every swing until then). `mode=track` fires the pawn's own input event nodes per changed bit of the streamed track (Jump 20/21, Attack 4, Crouch 16/15, WallRide 13/14, Throw 7, Guard 9, Interact 10, LockOn 11/12; Power, QuickMap, Perspective never), puts `cam_yaw/cam_pitch` on the ghost's AIController, feeds the stick through `AddMovementInput` in that frame AND fires the Move node AND writes `inputVectorWorld`/`moveInputAmount`/`hasMovementInput?` (the game's own stand-up-from-a-chair reads the rising edge of those; an explicit EndInteract-on-stick was wrong for the table glitch), and snaps the pawn to the recorded position past `snap` units (150). `mode=attack_loop|attack3|attack4|attack5|jump|fn:<name>` are the census modes. Seen: jump, floor, swing, the recorded route walked and faced with the walk animation, the chair sit, the seated-walk glitch. Open: Move's own handler cannot move a clone (bound value), corrections spike on the clip's cling at its end. The hurt-variant sit was the nulled reference to the pawn's OWN `BP_HpHitable` component (user-confirmed on the Lua prototype 2026-09-09; the DLL restores it at prepare, unwatched); `private_gi` stays as a harmless copy. The stand-up is the open one (`stand_fn`). Record: `UNVERIFIED.md`, `phases/phase7.md` 2026-09-09. |
| `INPUT_TRACK_AXES` | **Added 2026-09-08 with the flag above.** Four axes on every edge (`move_x, move_y, look_x, look_y`), quantized to 1/64 and throttled to `INPUT_TRACK_AXIS_MIN_MS` unless a button edge carries them: move from `IA_Move`'s bound value (the one action this pawn value-binds), look from `GetInputVectorKeyState` summed over the keys the applied table binds to `IA_Look` (stick position plus mouse delta -- the third run read the bound value as 0 for a whole run of looking). **Six axes from the 2026-09-08 evening build (ADR 0057, Stage 0), UNWATCHED: `cam_yaw`, `cam_pitch` -- the camera's yaw and pitch in degrees, absolute, from the camera manager (`camera_world_rotation`, the read the recording indicator is placed by), the camera frame a driven ghost's Move handler needs, which the look deltas cannot give. **Not the controller's `ControlRotation`: the first build read that and it sat at -90/0 through a full circle (21:39 run) -- this game drives its own `BP_PlayerCam_C` rig and never moves the controller's rotation.** `source` becomes `imc_keys+bound_axes+camrot`; a transient miss holds the last value; a camera that never answers in 600 samples logs `INPUTTRACK camera refused` once, sends the six slots with the last two at 0 and keeps the old `source`, so a reader can tell. The declaration line waits for that first answer.** `false` sends buttons only. |
...
| `GHOST_DESTROY_ORPHAN_CAMERA_RIGS` | **Added 2026-09-06 — a ghost's `BP_PlayerCam_C` camera rig is DESTROYED with the ghost (`release_ghost`, matched by `OwningActor`), and the sweep destroys any rig whose `OwningActor` reads null for `CAMERA_RIG_ORPHAN_SWEEPS_BEFORE_DESTROY` consecutive sweeps.** The leak the user reported as "fps drops after every despawn, fine in the pause menu, fine after reset to last save": the census probe found every ghost object collected within 90s except the rig, and 68 orphans cost ~1.7 ms a frame uncapped (`UNVERIFIED.md`, 2026-09-06). `false` leaves the rigs alive and only neutralised, i.e. the pre-2026-09-06 behaviour. |
...
days, which is why the figure carries a date and the COMMAND, never a total to trust): **102**
`constexpr bool` declarations in `Plugin.cpp` (2026-09-08: 97 plus the three input-track flags, `REC_INDICATOR_SCREEN_SPACE` and `INPUT_HISTORY_DISPLAY`), being 101 written plus
...
| `ghost_drive.txt` | `key=value` lines: `mode=` (`track`, `attack_loop`, `attack3..5`, `jump`, `fn:<function>`), `gap=` seconds, `block=` calls, `tail_until=` 0..6 (the tail bisect: how much of the per-ghost tail a driven ghost still runs), `snap=` units, `private_gi=` 0/1 (a private game-instance object for the driven ghost, 2026-09-09: the hurt-variant sit), `power_press=`/`power_release=` IA_Power node index 0..2 or -1 (the press index is UNMEASURED; default 1 by the Attack analogy), `mirror_offset=x,y` units (every OTHER replay ghost drawn offset while armed: the side-by-side), `stand_fn=<function>[:<byte>]` (called on the stick's rising edge while the driven pawn is seated; a no-arg function, or a one-byte one with `:<byte>`; DEFAULT `change Move State:0`, the pawn's own state setter with the state the clip shows after a sit -- it left the seat on the numbers 2026-09-09 11:56 where six interact/heal candidates did nothing; the unfiltered function-name dump named it, `probes/probe_pawndiff/`), `stand_stop_blend=<seconds>` (default 0.2; right after `stand_fn`, the pawn's `customStopMontage(blend)` -- the sit is a montage the state change leaves playing, and the player's own stand-up blends it out in 150-200 ms, measured 2026-09-09; negative = no stop), `move_gate=3,4` / `action_gate=18` (the states in which the player's own Blueprint does not pass the stick to the movement component, measured on the player 2026-09-09 with `input_gate.lua`; the rig skips its `AddMovementInput` there -- without it the clone rode up every wall it clung to; empty clears). A driven ghost's blob shadow is placed by the same per-tick `manageBlobShadow` call as a mirrored one's since 2026-09-09 (it used to sit inside the skipped mirror block: the shadow rode the model). `snap_stop=0/1` (default 0: a correction sets the recording's velocity after the teleport, direction from the target's own last step; 1 is the old `StopMovementImmediately`, which zeroed every Sunsetter plunge within a tenth of a second and restarted every wall slide, measured 2026-09-09 12:4x). `bound_stick=0/1` (default 1, 2026-09-09: a post-hook on the native `UEnhancedInputLibrary:GetBoundActionValue` answers the DRIVEN ghost's `IA_Move` reads with the recorded stick -- the track's `move_x/move_y` are that very value on the player -- so the pawn's own Blueprint moves, gates and steers itself; the rig's engine-level `AddMovementInput` and its stick-field writes are off. 0 = the pre-hook rig). The prepare also copies the save's numeric upgrades (`healUpgrades`, `damageUpgrades`, `powerBuildUpgrades`, `powerMeterUpgrades`, `bonusAirKicks`, `healAmountPerDing`) and the `can*` bools since 2026-09-09. Arms `GHOST_DRIVE` on the first replay ghost; removing the file hands it back to the mirror on the next tick. Dev only. |
...
| `ghost_parts_keep.txt` | **Keeps ON the pawn parts a ghost has no use for** (2026-09-06; `strip_ghost_no_use_parts`). Shipped behaviour switches off at spawn `SpringArm1` (the dialogue-camera boom), `DialogueCam`'s tick, the capsule's tick, and the ghost's `AIController` tick with its path-following and pawn-actions components; this file is the A/B, for `perf_report.txt` and for the user's eyes. Applies to ghosts spawned while it is present. |
| `ghost_charmove_off.txt` | **Also switches off the ghost's `CharacterMovement` tick at spawn** (2026-09-06). Opt-in, not shipped: whether the animation blueprint reads that component's velocity for walk/run/jump is a visual question, to be judged on a MOVING peer before it goes in the default set. |
| `ghost_fixlights_on.txt` | **Re-enables the `FixAllLights` repair after ghost spawns, which no longer runs by default** (2026-09-06). The repair measured 17.5-18.2 ms per call and ran once per spawn (18 of the 19 ms a spawn cost on our side, eleven of them in one frame at "play"). With the ghost's light killed on the spawn tick it had nothing left to repair: the user watched fake peers spawn beside them in a dark area with it skipped -- *"they stay dark even inside a dark area"*, and when asked for a straight answer: *"its fixed"* (`VERIFIED.md` 2026-09-06) -- so it is off. Present, it runs deferred and coalesced (once per burst, at most every `FIX_LIGHTS_SPACING_TICKS`). |
...
|---|---|---|---|
| `g_hud_x` .. `g_hud_z` | `26 22 44 10 30 4 0` | The screen-space indicator's placement and sizes in 1920x1080 pixels: clock right-edge inset, top inset, square side (only until the box has laid out; then the box's height), gap, digit font size, box padding, z-order | **Judged by the user against the old indicator on the Lua prototype, ShareX-checked (2026-09-08)** |
...
| `REGISTRY_RESEED_TICKS` | `600` | The object registries' belt: how often each re-seeds itself with a real `FindAllOf` so a member no construction callback covered is late by at most this (~4 s at 144 Hz) | **Sized** 2026-09-06; the walk is ~1 ms, so 1/600 of one per tick |
| `GHOST_SPAWN_SPACING_TICKS` | `2` | Ghost spawns are at least this many ticks apart, so a lobby joins over a quarter second instead of one frame | **Sized** 2026-09-06 (the user's ask); the engine clone is ~2.5 ms per spawn |
| `FIX_LIGHTS_SPACING_TICKS` | `30` | When `ghost_fixlights_on.txt` re-enables the light repair, how often at most it runs (one call per burst of spawns) | **Sized** 2026-09-06; the repair is 18 ms |
| `NAMETAG_GAP_ABOVE_OUTFIT` | `34` | The air between an outfit's declared top and its nametag -- the stock outfit's own gap under the old fixed height, so the stock outfit did not move | **Measured** 2026-09-06 (stock top 76 above the actor, tag at 110) |
| `NAMETAG_OUTFIT_TOP_MIN` / `_MAX` | `40` / `400` | The window an outfit asset's declared top must fall in to place the tag; outside it the fixed height stands (one mod asset declared 13,558) | **Sized** 2026-09-06 |
| `g_ghost_range_throttle` / `_far` / `g_ghost_range` | `6500` / `8500` / `10500` | The distance tiers' defaults in code, overridden live by the three `config.json` keys | **Measured** 2026-09-06 -- the user's two marks in a castle hall (`VERIFIED.md`) |
