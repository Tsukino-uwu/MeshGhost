...
build number read from memory** — no shipped adapter reads one (there's no cited
...
that did not advertise the identical list. Every shipped adapter omits it. What the planes are:
...
- **`session_policy`** carries the room-wide policy the host set, resolved by the core against this
  ...
  **One thing outranks it (2026-09-03, ADR 0047): a `render_remote` carrying `"cosmetic":true`
  ...
  solid checks the flag before it does. `session_policy` may also carry `"chaser_contact":"hurt"`
  or `"kill"` (ADR 0068; absent when off): the one effect a chaser may have — an overlap that does
  exactly what an enemy's touch does, or a guaranteed death — which you honour ONLY under a
  per-game ADR of this adapter's own and the user's on-screen confirmation, by triggering the
  game's own damage or death path, never by writing health — absent both, ignore it.
...
`extras` is load-bearing, not decorative — every shipped adapter relies on it
...
- **Clear the remote-ghost map on connect *and* on disconnect.** `despawn_remote` is not
  ...
  peer's ghost stands frozen on screen indefinitely. Every adapter hit this live.
...
Two more traps if your ghost is a clone of the real player, which is how every adapter
