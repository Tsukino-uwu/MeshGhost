...
// point of internal/netx is that tcp, udp and quic are interchangeable behind
// one interface — agent_docs/contract.md promises `send` is "reliable AND
// ordered on every transport", and internal/relay is deliberately built so it
...
// rather than block forever. internal/transport's read loop turns that error
