...
// internal/relay's Serve takes a net.Listener and handleConn a net.Conn, so
// neither changes to gain a transport; internal/transport wraps whatever
...
// which Client.gateMu's comment in internal/relay leans on when it says
...
//
// This package deliberately has no dependency on internal/protocol,
// internal/core, or internal/relay, the same leaf-package discipline
// internal/transport keeps.
//
...
import (
	...
	"meshghost/internal/netx/quicconn"
	"meshghost/internal/netx/udpconn"
)
...
const (
	...
	// — see internal/README.md's transport section.
	TCP Kind = iota
	...
	// Dial refuse it — internal/core resolves it to a concrete Kind before
...
func Dial(k Kind, addr string, timeout time.Duration) (net.Conn, error) {
	...
	case Auto:
		return nil, fmt.Errorf("netx: %q must be resolved to a concrete transport before dialing (internal/core does this via relay discovery)", Auto)
	default:
		return nil, fmt.Errorf("netx: unknown transport %v", int(k))
	}
}
