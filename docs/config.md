...
| `chaser` | `{"enabled": false, "count": 1, "delay": "3s", "spacing": "2s", "name": "", "color": "", "contact": false, "spawn_delay": "0s"}` | Your own past following you: `count` ghosts, the first `delay` behind and each next one `spacing` further back. A chaser appears only once you have been moving for `spawn_delay` (`0s` = its own delay), so none spawns on top of you at the start. Always cosmetic, whatever `ghost_collision` says. `contact` tells a game's mod a chaser may hurt on touch; no shipped mod honours it yet. |
