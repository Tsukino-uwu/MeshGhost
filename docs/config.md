...
| `interp` | `250ms` | How far behind real time other players' ghosts are drawn, which is what makes their motion smooth. Never set it below your connection's jitter; each game's mod may ship its own value. |
...
| `replay` | `{"record_on_launch": false, "save_last": "30s", "start_delay": "0s", "seek": "5s"}` | Recording and playback (2026-09-03). `record_on_launch` writes every session to the `replay` folder beside this file, from your first in-game moment to quitting; `save_last` is how much recent play the save-last key writes out after the fact; `start_delay` is how long after you are in the game a replay ghost starts (a file's own `start_delay` overrides it); `seek` is one rewind or fast-forward press. Drop a file into `replay/active/` and it plays as a cosmetic ghost; nothing here names a file. |
| `chaser` | `{"enabled": false, "count": 1, "delay": "3s", "spacing": "2s", "name": "Chaser", "color": "#7A2A2A", "contact": false}` | Your own past following you: `count` ghosts, the first `delay` behind and each next one `spacing` further back. Always cosmetic, whatever `ghost_collision` says. `contact` tells a game's mod a chaser may hurt on touch; no shipped mod honours it yet. |
