...
| `replay` | `{"record_on_launch": false, "save_last": "30s", "start_delay": "0s", "seek": "5s"}` | Recording and playback (2026-09-03). `record_on_launch` writes every session to the `replay` folder beside this file, from your first in-game moment to quitting; `save_last` is how much recent play the save-last key writes out after the fact; `start_delay` is how long after you are in the game a replay ghost starts (a file's own `start_delay` overrides it); `seek` is one rewind or fast-forward press; `gzip` (on) writes recordings as `.ndjson.gz`, about 30x smaller than an untrimmed plain file and read identically -- turn it off if you want a file you can open in a text editor; `split_times` (off) puts how far behind or ahead of the ghost you are on its nametag, e.g. `PB +1.2s`. Drop a file into `replay/active/` and it plays as a cosmetic ghost; nothing here names a file. |
