...
| `ghost_range_throttle`, `ghost_range_far`, `ghost_range` | `3000`, `5000`, `10500` | Read by the game's mod (Pseudoregalia as of 2026-09-06), in that game's own units: how far a ghost can be before it is drawn cheaper. Beyond the first, its animation runs at the engine's reduced rate; beyond the second, its pose freezes while it still moves; beyond the third it goes dormant — hidden and ticking nothing, exactly like a despawn to look at, but back in one frame when it comes closer. `0` disables that step. Re-read while the game runs, so you can tune them without a relaunch. |
