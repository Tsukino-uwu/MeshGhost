...
## 2026-09-05 — the chaser gets gameplay time, and a tap sized to what the adapter actually sends

**Backfilled 2026-09-11** — the day's detail is in [phase10.md](phase10.md)'s 2026-09-05 entry;
...
most 100 samples a second — **the queue had been sized for that while the adapter sends ~180**, so
the sizing and the source disagreed about the same number.

## 2026-09-07 — the chaser pack shares one history
...
`c.mu` rather than bare — the same settings-race class the 2026-09-09 entry below closes for the
whole config.
