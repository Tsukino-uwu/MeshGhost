...
`list_menu_probe`, `trainer_approach_probe`, `printer_state_probe`, `naming_probe`, `task_probe` and `battle_state_probe` (gaining the script pointer,
...
ability and WONDER GUARD (the user); the MACH BIKE, SELECT to mount, and the map header's cycling bit. All in `adapters/emulator/pokemon/emerald/MEASURED.md`, same date; the autoplay side
