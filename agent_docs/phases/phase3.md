...
- **LuaSocket, not BizHawk's built-in `comm.*`.** Both were inspected against the real
  ...
  keeps the bridge as plain NDJSON. Vendored: `adapters/bizhawk/pokemon/emerald/lib/x64/socket-windows-5-4.dll`
...
- [x] Vendor the LuaSocket binary + MIT license notice.
      `adapters/bizhawk/pokemon/emerald/lib/x64/socket-windows-5-4.dll` + `luasocket.LICENSE.txt`.
...
- [x] Write `adapters/bizhawk/pokemon/emerald/probes/phase3_loopback.lua`: real state reading (same addresses as
...
- [x] **First live BizHawk attempt failed** loading `socket-windows-5-4.dll`:
      ...
      inside the user's BizHawk (`adapters/bizhawk/pokemon/emerald/lib/x64/lua54.dll`) and pre-load it by full
