...
(two of two) and not the client (none of seven, the seventh scoring Microsoft only); why is unmeasured.
**Filed at the close:** the method lesson in `pitfalls/method.md` (a scanner verdict is per file),
indexed and on the fix checklist; the `risks.md` antivirus entry carries the day's outcome; the
branding sketch closed in the `ideas.md` entry. The user closed this chat and opened a new one. The 0/70 confirmation failed: the flags are not a lever on Defender, and Bkav hit an
