...
- **Prove the file still does what it did BEFORE, not just that your change landed.** A slice that removes a block can take the statement below it: an edit to a Lua adapter took a function's `return` with it, so it returned nil and the frame guard swallowed the error every frame -- no ghost on either screen, found by the user and not by the forward-reference check I had just run (2026-09-13). The cheapest proof is what the file PRODUCES: a load line, a return value, a parse.
...
- **An inline shell heredoc mangles backslash escapes on this machine** (2026-09-08): `\n` inside a Python patch written through the Bash tool reached the source as a real newline, and `\x00b7` as a NUL byte plus the text `b7`, twice in one afternoon -- write the patch script to a file with the Write tool and run it by path, and grep the result for the literal you meant. **Repeated 2026-09-23, three times in one session** (a C++ log string and a Lua print, each caught only by the compiler or `luac -p`): the rule was on this page and inline heredocs were used anyway.
