...
- **A timing assertion must derive every number it compares from ONE reading of the clock (2026-09-04).** `TestALocalGhostStillInterpolatesRatherThanEdgeHolding` placed its samples relative to one `nowMs()` and computed its render time from another, so a millisecond boundary falling between the two moved the answer by one. It passed locally for a day, and failed on a runner -- x=-21 where -22 was wanted -- **in the release workflow, after CI had already gone green on the same commit**. The fix is not a tolerance band: take the reading once, derive both ends from it, and the test is exact again. A band would have hidden the next real off-by-one.
