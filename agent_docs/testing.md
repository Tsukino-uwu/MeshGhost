...
| `FuzzValidateInputSampleAgreesWithItsOwnRejectReason` (bridge) | `ValidateInputSample` and `InputSampleRejectReason` are two hand-maintained ladders over one set of rules, and nothing else keeps them in step: a rule in only the first refuses a batch with an empty reason in the log, a rule in only the second describes a check that no longer runs. Anything accepted is also re-checked against the constants directly, so a validator that loosened could not pass by agreeing with itself. **The first target in `bridge` that calls anything in `bridge`** — the two above exercise `encoding/json` — and the first anywhere to make this validator the subject rather than the oracle (X2-3/X2-7, 2026-09-12). |
...
| `FuzzMessagesNeverPanic` (pake) | Arbitrary bytes as the relay's incoming KE1 and as the client's incoming KE2 (ADR 0067): each side answers `ErrRefused` or a real message, never a panic. The relay's login endpoint is reachable by anyone who completes a TLS handshake, which is everyone. Seeds: the three genuine messages, empty, all-0xff, a truncated KE1. Wired the day it was written (2026-09-15). |
| `FuzzKnownRelaysFileNeverPanics` (core) | Arbitrary bytes as the client's `known_servers.json` (ADR 0066): the loader returns an error or a store, never panics, and what it loads round-trips through its own writer. A file this process writes and a hand may edit; 11 seeds including a valid file, wrong versions, an empty address and a non-fingerprint. Wired the day it was written (2026-09-15). |
...
- **A test that listens on a wildcard address puts a Windows Firewall prompt on the user's screen, per test binary (2026-09-15).** Every compile is a new executable, and Windows asks about each one that opens `0.0.0.0` or `[::]`; a gate run became a stream of "accept network" prompts. `127.0.0.1:0` for every test listener, and `grep -rn '"0.0.0.0\|"\[::\]' --include=*_test.go` before a gate. If a test genuinely needs the OS's answer for a wildcard, it asks the user first.

- **A test that reads a message the server sends right before CLOSING is Linux-flaky by construction, unless the close is graceful (2026-09-05).** `TestRateLimitedClientReceivesRejectBeforeClose` timed out once on the release job's Linux runner and never locally: the relay closed a socket still holding the client's unread flood, the kernel sent a reset, and the reset discarded the Reject in the client's buffer. Fixed in the relay (`transport.CloseGracefully`), not in the test. A "message then close" test that flakes on Linux only: suspect the close, not the timing -- `agent_docs/pitfalls/by-lesson.md`, 2026-09-05.

- **A timing assertion must derive every number it compares from ONE reading of the clock (2026-09-04).** `TestALocalGhostStillInterpolatesRatherThanEdgeHolding` placed its samples relative to one `nowMs()` and computed its render time from another, so a millisecond boundary falling between the two moved the answer by one. It passed locally on 2026-09-04 and failed on a runner that same day -- x=-21 where -22 was wanted -- **in the release workflow, after CI had already gone green on the same commit**. The fix is not a tolerance band: take the reading once, derive both ends from it, and the test is exact again. A band would have hidden the next real off-by-one.
...
- **A fuzz target explores what its PEER does, and ours has always been a well-behaved one (2026-09-06).** `FuzzEverything`'s alphabet has `attach` and `detach`, varies the chaser count and the roster, and drives the real `ServeBridge` -- and it could not have found the 2026-09-06 dead-adapter bug at any peer count, because the bug is not about COUNT. Its fake adapter always drains the socket promptly, so no write ever fails, no socket ever dies mid-frame, and the reconnect never races a close. `net.Pipe` compounds it: a pipe write blocks until the peer reads rather than filling a kernel buffer and expiring a deadline, which is the exact mechanism that broke. **The lesson is that "fuzz everything" has to include the OTHER SIDE misbehaving -- a peer that stops reading, a write that fails, a reconnect that arrives before the close is noticed -- not just our own order, values and timing.** What would catch it, and is not built: three ops (`adapter.stopReading`, `adapter.dirtyReconnect`, `adapter.slowReader`), a fuzzed short `bridgeWriteTimeout`, and one invariant -- a hello is never refused `busy` while the incumbent's socket is closed. `ideas.md`, 2026-09-06.

- **Scale belongs in the fuzzer only while it is CHEAP, and the line is measurable (2026-09-06).** Widening `FuzzEverything`'s peer space paid immediately: a flood of `MaxRosterSize + 88` joins costs almost nothing per execution, because a join with no state admits an id and renders nothing, and it found a deadlock the same evening. The same instinct applied to REPLAY clips did not: a zip of `MaxRosterSize + 40` clips put 512 local ghosts on the roster, each rendering over the bridge every tick, which took one execution from milliseconds to ~10 s -- 207 execs/s to zero, and the engine killed the worker as hung. **The test is per-execution cost, not how impressive the number is:** ask what each unit of the crowd DOES per tick. Where it is expensive, pin the scale in a plain test that runs once (`TestAZipOfMoreClipsThanTheRosterHasSeats`: 552 clips, 512 admitted, 0.04 s) and leave the fuzzer a small multiple.
