...
- 2026-09-15 — **Prediction groundwork LANDED (`d53291a5`, ADR 0069): the `correction` knob ships `0s`; the SCREEN VERDICT IS OPEN** — two real peers on netsim no-arg, `interp 450ms`/`extrapolate 100ms`/`predict damped`/`correction 100ms`; floor-sink in a gap is expected (Track B). A2 waits on p95/p99 transit + a dry-gap histogram on the stats line. `prediction-planning.md`, `phases/phase10.md`.
- 2026-09-15 — **Chaser contact: the Go half LANDED (`e546d38c`, ADR 0068: `chaser.contact` is `off`/`hurt`/`kill`, ships `off`); the adapter half is NOT started.** Next: Part A of `chaser-planning.md` — load the parked `probe_hitlist` on a loopback ghost and measure the Sunsetter/Strikebreak/lever leaks; `pseudoregalia/UNVERIFIED.md` has the entry.
