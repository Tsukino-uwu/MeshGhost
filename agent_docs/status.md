...
- 2026-09-04 — **OPEN, unconfirmed:** chaser ghosts may spawn/despawn unintentionally (watched second-hand; a gap in the middle, not a cap). First fork is free: diff the core's admit/drop log against the adapter's spawn/release — `adapters/pseudoregalia/UNVERIFIED.md`.
- 2026-09-04 — **MEASURED, three Pseudoregalia replay defects, none fixed.** A ghost's sword is `WeaponMesh.bVisible` and only TRANSITIONS are applied, so a peer who never held one shows one — `adapters/pseudoregalia/UNVERIFIED.md`.
- 2026-09-04 — World-spawned VFX are UNOWNED and outlive the ghost (`SpawnSystemAtLocation`): the left-behind sword glow, the stranded dust and the poisoned `observed_world_offset_z` are ONE defect — same queue.
