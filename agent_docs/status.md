...
- 2026-09-05 — **The chaser despawn/respawn cycle is REPRODUCED and FIXED in the core: the chaser queue was sized for 100 samples/s and the adapter sends ~180, so every chaser more than ~6s behind lost a seam-sized hole every delay+3s; the tap now thins to 100Hz, regression-tested, core deployed, UNWATCHED** — `pseudoregalia/UNVERIFIED.md`.
...
- 2026-09-05 — **The ghost sword mirror read `bVisible` as a plain byte (the bitfield trap, third time) and rewrote+logged it EVERY TICK per ghost (~140 lines/s); fixed with `mg_read_bool`, built, deploys at the next game exit, UNWATCHED** — 65 other `mg_property_value<bool>` reads want the same audit — `pseudoregalia/UNVERIFIED.md`.
