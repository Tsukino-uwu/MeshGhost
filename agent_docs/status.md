...
- 2026-09-04 — **OPEN, user- and tester-reported:** dust VFX land in wrong positions after a replay ghost restarts/loops, *"somewhat related to the height of ghost sybil when it despawns"*. Lead, unmeasured: the ghost-effect exclusion dies with the `remotes` entry while the component outlives it, so a despawning ghost's dust can poison the file-scope `observed_world_offset_z` — `adapters/pseudoregalia/UNVERIFIED.md`.
