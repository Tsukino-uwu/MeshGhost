...
- 2026-09-17 — **Autoplay (Phase 13): Phase 1 DONE, at its CHECKPOINT** (Emerald: a new game to MAY's battle walked, one run log); next: the scenario runner (moved up at Phase 0's checkpoint). Crystal (its own chat, paused): observe, `walk`, `select`, a walked starter and a wild battle on vanilla V1.0; next the battlers in memory, then shared `advance_text` (user's call). `phases/phase13.md`.
