...
- 2026-09-05 — The FROZEN-PLAYER defect: **chaser half built (ADR 0053); the PROBE is done — the pause menu and the item popup are both the engine's pause (`WorldSettings.PauserPlayerState`), the cutscene is a scripted possession with no Lua-readable edge**; next is the adapter's `player_frozen` send on that field. Still **blocks `chaser_contact`** — `pseudoregalia/UNVERIFIED.md`, `PROBES.md`.
