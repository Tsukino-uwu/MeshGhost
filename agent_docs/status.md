...
- 2026-09-05 **v1.1.6 SHIPPED (2026-09-05, from `8e47e01b`, all four path-filtered workflows green; v1.1.5 earlier the same day, v1.1.0 2026-09-04). Player-facing change: client + config move to the game folder; upgraders move two files up. Three things wait on the user's eyes in Pseudoregalia: a 3s chaser should now LOOK 3s behind (ADR 0049), a recording should start at the first real frame rather than in the menu, and the player's own SFX going quiet near a ghost is OPEN and undiagnosed** — `pseudoregalia/UNVERIFIED.md` is the queue, newest first.
