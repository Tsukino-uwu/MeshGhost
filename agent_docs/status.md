...
- 2026-09-17 — **Autoplay (Phase 13): Phase 1 DONE; the scenario runner BUILT** (`cmd/scenario`: RICK's sight replayed 3 of 3 with no model, failing when broken); next on Emerald: the STRING SHOT message, keyboard/clock/starter-bag readers, `exec`, noclip. Crystal (its own chat, paused): observe, `walk`, `select`, `advance_text`, `battle run` on vanilla V1.0 (the text machine is shared Lua now); next the battlers in memory. `phases/phase13.md`.
