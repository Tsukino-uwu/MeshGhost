...
- 2026-09-05 — The FROZEN-PLAYER defect: **the chaser half is decided (ADR 0053, chaser only, by the user's call) and built on the Go side with a regression test**; the adapter half is a PROBE for Pseudoregalia's own frozen signal, then a `player_frozen` send, then a chaser watched across a pause menu. Still **blocks `chaser_contact`** until that lands — `pseudoregalia/UNVERIFIED.md`.
