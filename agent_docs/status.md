...
- 2026-09-17 — **Autoplay (Phase 13): Phase 1 DONE; the scenario runner BUILT** (`cmd/scenario`: RICK's sight replayed 3 of 3 with no model, failing when broken); next on Emerald: the STRING SHOT message, keyboard/clock/starter-bag readers, `exec`, noclip. Crystal (its own chat): observe with the battlers, `walk`, `select`, `advance_text`, `battle strongest` and `run` on vanilla V1.0; next a trainer battle (Route 30), then the PACK's list. `phases/phase13.md`.
