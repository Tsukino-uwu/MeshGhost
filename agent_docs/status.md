...
- 2026-09-05 — The FROZEN-PLAYER defect: **BOTH halves built (ADR 0053) — the core's chaser clock stops on `player_frozen`, and the Pseudoregalia adapter sends it from `WorldSettings.PauserPlayerState` (the pause menu and the item popup, per `probe_frozen/`); UNWATCHED** — a chaser held through a pause is the look. Still **blocks `chaser_contact`** until watched — `pseudoregalia/UNVERIFIED.md`.
