...
- 2026-09-05 — The FROZEN-PLAYER defect: **chaser half built (ADR 0053); the PROBE ran and the pause menu is the engine's own pause (`PauserPlayerState`)**; the item popup, the intro cutscene and a zone transition are unmeasured, then the `player_frozen` send. Still **blocks `chaser_contact`** — `pseudoregalia/UNVERIFIED.md`, `PROBES.md`.
- 2026-09-05 — **RESET TO LAST SAVE crashed three times (03:56, 04:15, 04:22), reproducible by spamming the pause menu first; the cause is the reset-button HOOK itself (it crashed with a do-nothing callback), which only ever ARMED by accident — the hook is no longer registered, built and deployed, UNWATCHED** — rule: `checklists/before-spawning-in-unreal.md`; `pseudoregalia/UNVERIFIED.md`.
