...
- 2026-09-04 — Replay hotkeys, from a tester using them: chords are required and there is no feedback outside the console — both OPEN in `ideas.md`. The third complaint, that the log did not say whether a record toggle STARTED or STOPPED, is fixed: `ReplayControl` returns what it did.
