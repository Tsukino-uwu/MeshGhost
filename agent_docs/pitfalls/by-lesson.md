...
- **Say out loud that a writing probe is loaded, every time.** The user was mid-test on an
  ...
  commit that had just landed. `PROBES.md` already lists which probes write RAM for exactly
...
## Archipelago coexistence on Emerald: fixed EWRAM addresses shift under the patch, pointer-relative reads do not (2026-08-11 → 2026-08-18, moved out of risks.md 2026-09-02)

**Moved out of `agent_docs/risks.md` on 2026-09-02**: a 168-line risk bullet that was an incident record
(symptom, the shift table, the source-level cause, the third address family it hit, and how the
vanilla-only `inOverworld()` check was found to be the real bug and fixed). The three-line risk that
replaced it names the standing constraint; this is the narrative, verbatim.
...
## A file whose first line said "deliberately untracked" was committed anyway -- a header is prose, and prose is not a gate (repo, 2026-09-09)
**Symptom.** `REVIEW-FINDINGS.md`, a session working checklist headed *"deliberately untracked (not committed, so it cannot trip the doc/preflight gates)"*, was committed on 2026-09-08 with the message "track the findings checklist" and sat on the public repo until the user found it on 2026-09-09 and asked why. Its own header, `status.md` and the phase file all still called it untracked.
