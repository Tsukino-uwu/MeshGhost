...
- **A statically-embedded scripting runtime inside a host binary is a higher-risk target for
  ...
  a staged probe (see Diagnostic methodology in [method.md](method.md)) rather than a "if it loads, it's fine"
...
- **Process note, transferable beyond UE**: this change also swapped a *confirmed-working*
  ...
  the "one diagnostic at a time" rule in [method.md](method.md) — which regressed a working visual
...
- **The wrong turn that cost the most**: an A/B set `AFTERIMAGE_TRIGGER_OBSERVED = false` and
  ...
  revert** (the rule's home is [method.md](method.md), indexed in [../pitfalls.md](../pitfalls.md)).
...
- **Generalizes to** — the rules extracted from this are in `## Diagnostic methodology`
  ([method.md](method.md), indexed in [../pitfalls.md](../pitfalls.md)), but the short form: audit a
  probe's cost before trusting its output; never leave a spawning probe enabled while judging what
  it spawns; re-test with probes off; a flag flip is not a revert; and bisect real commits early
  rather than reasoning about a regression.
