...
- **Archipelago coexistence, confirmed with a real gap (2026-08-11, CLOSED for rendering 2026-08-14).**
  Under the patch, fixed EWRAM/IWRAM addresses (`gPlayerAvatar`, `gObjectEvents`, `gMain`) read garbage while
  `gSaveBlock1Ptr`-relative reads are fine: derive facing from position deltas, never read it. The full
  record — the shift table, the source-level cause, the `inOverworld()` bug it predicted — is
  `pitfalls/by-lesson.md` (moved 2026-09-02). Standing constraint: a recompile can move any fixed address.
...
- **A maximal `event`/committed `escrow_state` exceeds a udp datagram and is silently dropped** —
  pre-existing, unreached today, and its own decision to fix.
