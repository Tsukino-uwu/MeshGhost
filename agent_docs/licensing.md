...
| [Rain-Meadow](https://github.com/henpemaz/Rain-Meadow) | **NONE — no license file** | **Checked 2026-08-24 via the GitHub API (`repos/henpemaz/Rain-Meadow` returns `"license": null`), not from memory or the README.** No license means all rights reserved by default, so this is a **no**: do not read its source, do not derive from it, do not use it as an implementation reference. Naming and linking it here is fine — an unlicensed public repo is a softer no than a private one (`CLAUDE.md`), which may not be named at all. Re-check before any future use: an unlicensed repo can gain a license at any time, and this row is true as of its date only. |
...
| [quic-go](https://github.com/quic-go/quic-go) | MIT | Checked 2026-08-16 via `gh api repos/quic-go/quic-go/license` (`.license.spdx_id == "MIT"`), reading the repo's own license file rather than the badge — the Archipelago/GBA-PK lesson that the two can disagree. Permissive, compatible alongside this project's MIT `LICENSE`. **Checked ahead of reading any of its source**, per the standing rule, because the selectable-transport work (`transport: tcp \| udp \| quic`) would make it **this repo's first third-party Go dependency** — a `go.mod` module requirement, not vendored source, so nothing of it would sit in the tree. That distinction matters for the publishability rule: a `require` line plus `go.sum` hashes is a reference to code fetched at build time, not a copy of it. Two things to settle before it actually lands: whether CI can fetch it (it currently builds with an empty module graph), and whether a larger binary with a real dependency shifts the antivirus false-positive baseline recorded in `risks.md`. Not yet a dependency as of this entry — recorded at scoping time so the check does not gate implementation later. **Its own dependencies come with it**, since a Go module is compiled *into* our binary and both MIT and BSD-3-Clause require the notice to travel with a binary — so this needs a `THIRD-PARTY-NOTICES.txt` for the Go executables, the same treatment `UE4SS.dll` already gets. Checked 2026-08-16 by reading quic-go's own `go.mod`: the linkable set is quic-go itself, `github.com/quic-go/qpack` (**MIT**, `gh api repos/quic-go/qpack/license`), and four `golang.org/x/*` modules (crypto/net/sync/sys) which share Go's **BSD-3-Clause**, `Copyright 2009 The Go Authors` (confirmed for `golang.org/x/crypto`; confirm each of the others individually when they actually link in). `testify`, `go.uber.org/mock` and `gcassert` are test/tool dependencies and never reach our binary. **Do not take the notices list from `go.mod`** — `go version -m meshghost.exe` reports what was actually linked, which is the authoritative list and the one to audit against. Re-check the whole set whenever the quic-go version moves, same standing note as the RE-UE4SS pin. **Adopted as a real dependency 2026-08-16** at **v0.61.0**, for the `quic` transport. What actually links was then read from the built binary (`go version -m meshghost.exe`), not from `go.mod`, and is **smaller than go.mod suggests**: only `quic-go` (MIT), `golang.org/x/crypto` and `golang.org/x/sys` (both BSD-3-Clause, `Copyright 2009 The Go Authors`, each confirmed by reading the licence file rather than the badge). `golang.org/x/net` is required by `go.mod` but does **not** link, and `qpack`/`testify`/`go.uber.org/mock`/`gcassert` never reach the binary at all. Notices now ship at `packaging/release/THIRD-PARTY-NOTICES.txt` (separate from the UE4SS one, which covers a different binary), with the licence texts quoted verbatim from their own files. `go get` also raised the module's Go directive from 1.22 to 1.25.0. **Bumped to v0.62.0 on 2026-09-02**: it requires Go 1.26, so `go get` raised the directive to 1.26.0 and both workflows' `setup-go` pins followed; no source change; the linked set re-read from both binaries with `go version -m` is unchanged (quic-go, `x/crypto` v0.54.0, `x/sys` v0.47.0), licence still MIT (`gh api repos/quic-go/quic-go/license`, same day). |

| [BepInEx.Debug](https://github.com/BepInEx/BepInEx.Debug) (`ScriptEngine`) | LGPL-3.0 | Checked 2026-08-28, via `gh api` **and** by reading the repo's own `LICENSE` text directly (`GNU LESSER GENERAL PUBLIC LICENSE Version 3`) rather than the badge, per the Archipelago/GBA-PK lesson. Repo active, release `r11.1` (2026-01-01). **A developer-machine tool, not a dependency and never shipped** — the same posture as BizHawk and ILSpy: it is dropped into the developer's own BepInEx install, MeshGhost neither links against it nor redistributes it, and no shipped artifact changes. What it does: reloads plugin DLLs from `BepInEx/scripts/` while the game runs, which is what removes the relaunch from TEVI's edit loop (`dev-scripts/tevi-hotreload.ps1`). Its config key names were read out of the shipped `ScriptEngine.dll` with `ilspycmd` rather than from a wiki, because a repackaged build documented elsewhere lists the same keys under different sections — facts only, nothing copied. |
| [UnityExplorer](https://github.com/yukieiji/UnityExplorer) (`yukieiji` fork) | GPL-3.0 | Checked 2026-08-28, via `gh api` and by reading the `LICENSE` text directly (`GNU GENERAL PUBLIC LICENSE Version 3`). **The upstream `sinai-dev/UnityExplorer` is ARCHIVED** (last pushed 2023-03-13) and this maintained fork is what is actually installed — `v4.13.6`, 2026-04-30, asset `UnityExplorer.BepInEx5.Mono.zip`. A runtime object browser for the live scene tree. **Developer-machine tool, not a dependency, never shipped**, same posture as the row above; GPL's copyleft reaches code linked into it, and nothing of MeshGhost's is — it loads as its own separate BepInEx plugin beside ours. Its source is not read, only its released binary run. |
| [dnSpyEx](https://github.com/dnSpyEx/dnSpy) | GPL-3.0 | Checked 2026-08-28, via `gh api` and by reading the `LICENSE` text directly. **The original `dnSpy/dnSpy` is ARCHIVED** (last pushed 2020-12-20, and its API reports no license at all); this maintained fork is what is installed — `v6.6.0`, 2026-06-20, at `C:\dev\tools\dnSpy`, entirely outside any game folder. A standalone .NET debugger/decompiler run as its own process; nothing links it and nothing ships it. **The companion `dnSpy-Unity-mono` patched-runtime repo is NOT used and cannot be**: its newest build is Unity 2020.3.33f1 and TEVI is 2021.3.25f1 (checked 2026-08-28, 318 entries, no 2021 build). TEVI is debugged instead through BepInEx's own `doorstop_config.ini` `debug_enabled` flag, which needs no game binary replaced — see `environment.md`. |
| [Better-Super-Mario-Sunshine-Online (BSMSO)](https://github.com/Daytendo64/Better-Super-Mario-Sunshine-Online-BSMSO-) (`Daytendo64`) | GPL-3.0 | Checked 2026-09-04, via `gh api` **and** by reading the repo's own `LICENSE` text directly (`GNU GENERAL PUBLIC LICENSE Version 3`), not the badge — the user's read was *"its gpl3 i think"*, and this row is what turns that into a checked fact. Repo not archived, not a fork, last pushed 2026-08-08, language reported as **C#** (metadata, not a source read — worth one look before assuming a Dolphin fork, which would be C++). **Nothing of it may be copied, adapted, paraphrased or vendored, and none of it may ship.** GPL-3's copyleft would reach anything derived and this repo is MIT (`LICENSE`), so a derivation is not merely a licence question, it is a relicensing of MeshGhost — and the standing rule is stricter than the licence anyway: **facts may be used with a citation, expression never** (`access-models.md`). **Posture: read-only, and start OUTSIDE the source.** The one question worth taking to it is *where does it run and how does it draw* (see `candidate-games.md`'s Sunshine entry, where that is the recorded blocker), and README, releases and dependency metadata plausibly answer it without opening a source file — the same instinct as the UnityExplorer row's "source is not read, only its released binary run", which is the posture this table already applies to GPL work. If a fact does have to come out of the source, it is cited by file and line like every `pret` fact. **Not a dependency, not planned as one, and its existence is not a reason to read it.** It was recorded as evidence about DRAWING on that platform — but see the `BetterSunshineEngine` row directly below, which the user names as what it builds on: if the drawing is done by modifying the GAME, that is a route this project has ruled out, and the evidence points the other way from how it was first filed. |
| [BetterSunshineEngine](https://github.com/DotKuribo/BetterSunshineEngine) (`DotKuribo`) | GPL-3.0 | Checked 2026-09-04, via `gh api` and by reading the `LICENSE` text directly. Named by the user as what BSMSO above builds on: *"the online mod is using this ... for actually working i think"* — the dependency direction is theirs and is not verified here. Not archived, not a fork, last pushed 2026-07-31. **Same GPL-3 posture as the row above: facts with a citation, expression never, nothing vendored, nothing shipped.** **But the metadata alone carries the finding that matters, without opening a source file.** Its own description is *"A modification of the SMS engine"*, and its language breakdown is C++/C with **Assembly and a Linker Script** — the shape of code compiled for the console and injected into the game, not of an emulator plugin. Inference from metadata and the repo's own words, not measured. **If that holds, the mechanism is a door this project has closed**: `CLAUDE.md`'s "nothing that ships writes a save, game state, or a ROM patch — ever", and the emulator rule that adapters are Lua-only, which is what lets MeshGhost run on top of somebody's Archipelago seed. So this is prior art whose ANSWER we cannot adopt, which is worth knowing before anyone reads it, not after. |
| [archipelago-double-dash](https://github.com/aXu-AP/archipelago-double-dash) (`aXu-AP`) | MIT — read from the repo's own `LICENSE` file 2026-09-08 (`MIT License` / `Copyright (c) 2025 aXu-AP`), not the badge, per the Archipelago/GBA-PK lesson. | **Read-only design reference, not a dependency and never shipped** — the same posture as the CelesteNet and SilklessCoop rows, and stricter than MIT requires: facts with a citation, expression never. An Archipelago client for a GameCube title that needs **no patched disc image**: a Python process outside Dolphin writes assembled PowerPC into emulated RAM at boot. What was taken from it is the *shape* — read 2026-09-08 and written up in [`access-models.md`](access-models.md) as a fourth answer to the drawing question that section had left open — plus the names of the calls it makes into the memory library (row below) and the fact that it assembles with devkitPro's `powerpc-gekko-as`. No source text, no addresses, and none of its game-specific work is relevant here. **It does not settle whether the approach is open**: see the `BetterSunshineEngine` row above, which files the same mechanism as a closed door, and the paragraph in `access-models.md` that puts the two side by side. |
| [py-dolphin-memory-engine](https://github.com/henriquegemignani/py-dolphin-memory-engine) (`henriquegemignani`; bindings to `aldelaro5`'s Dolphin Memory Engine) | MIT — read from the repo's own `LICENSE` file 2026-09-08 (`MIT License` / `Copyright (c) 2017 aldelaro5`). | **Not a dependency and not used**; checked ahead of need, per the standing rule, because it is what the row above attaches to Dolphin with and any future GC/Wii work would start here. Python bindings for external-process access to Dolphin's emulated memory: `hook()`, `is_hooked()`, `read_bytes()`, `write_word()`, `write_byte()`, `un_hook()`, on x86_64 Windows/Linux plus macOS, against a **stock** Dolphin with emulation running — no fork, no plugin, no custom build, which is what keeps Dolphin's GPL out of the question entirely. Permissive and compatible with this repo's MIT `LICENSE` if it ever were adopted. **A 2026-09-08 snapshot of a project this repo has never run**: re-check, and confirm the API against its own documentation, before relying on any of it. |
...
| [go-winres](https://github.com/tc-hib/go-winres) | ISC — read from the repo's `LICENSE` file 2026-09-06. | **Build-time tool, pinned at v0.3.3, run via `go run` in `release.yml` and never vendored or imported.** It compiles `cmd/*/winres/winres.json` into the Windows version resource (`docs/code-signing.md`). Nothing of it ships: the `.syso` it emits is our own metadata, and the tool never touches the source. |
